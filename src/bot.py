...
class Bot:
    ...
    def add_commands(self):
        ...
        async def stats(interaction: discord.Interaction):
            ...
            embed = discord.Embed(
                title="Statistics",
                description="Honeypot Statistics",
            ...
            embed.add_field(
                name="Latest ping", value=self.db.get_latest_ping()[0]["ip"]
            )
        ...
        @self.bot.tree.command(
            name="joins", description="Get a list of latest join attempts"
        )
        async def stats(interaction: discord.Interaction):
            ...
            for join in joins:
                ...
                embed.set_author(name="MCHoneyPot")
                embed.add_field(
                    name=f"{join['name']}", value=f"{join['ip']}", inline=False
                )
                embed.add_field(
                    name=f":flag_{ping['country'].lower()}:", value="", inline=False
                )
        ...
        async def stats(interaction: discord.Interaction):
            ...
            for ping in pings:
                embed.add_field(
                    name=f'**{ping["ip"]}**',
                    value=f'> Country: :flag_{ping["country"].lower()}:\n> City: {ping["city"]}\n> ISP: {ping["isp"]}',
                    inline=False,
                )
